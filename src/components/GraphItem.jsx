...
const GraphItem = (props) => {
  ...
  const onAnalize = async (id) => {
    setLoading(true);
    try {
      const res = await GraphServer.getGraphbyIdf(id);
      const data = await res.json();
      console.log(data)
      navigate.push('/visualizacion', { data: data })
    } catch (error) {
      console.log(error)
    }
    /**;*/
        ...
        <div className="col-md-12">
          {
            loading ? (
              ...
              <div className="card card-body">
                ...
                <Button variant="primary" onClick={() => onAnalize(props.graph.id)} >
                  Analize
                </Button>
              </div>
            )
